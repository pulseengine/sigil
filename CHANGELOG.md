...
- **Bazel CI now runs `bazel test //...`** (audit C-7). Previously
  only `bazel build //...` was invoked, so Bazel-side test failures
  could merge undetected.
- **`continue-on-error` removed from formal-verification jobs that
  pass** (audit C-7). The Rocq job retains `continue-on-error` with a
  `# WIP — see audit C-3` comment because the directory is a stub.
